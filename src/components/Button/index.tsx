...
export const VzButton = ({
  ...
  const commonClassName = useMemo(() => `vz-btn vz-btn-${type} d-flex`, [type]);
  ...
  return (
    <div
      id={id}
      onClick={handleClick}
      className={`${commonClassName} ${className} ${selected ? 'vz-btn-selected' : ''} ${disabled ? 'vz-btn-disabled' : ''}`}
      style={{ ...style }}
    >
      {leftIcon}
      ...
      {!children
        ? (
          <div className={`vz-btn-label ${label && !labelClassName ? 'mx-2' : ''} ${labelClassName || ''}`}>
            {label}
          </div>
        )
        : children
